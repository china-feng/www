...
class download
{
    ...
    public function saveFile($fileName)
    {
        ...
        $siteFileLength = $siteFileInfo['Content-Length'] ? $siteFileInfo['Content-Length'] : 0;
        ...
        while (true)
        {
            ...
            //检查是否正常请求
            $code = $result['code'] ? $result['code'] : 0;
            if (!$code)
            {
            ...
            //返回流长度
            $streamLength = $result['length'] ? $result['length'] : 0;

            //返回流内容
            $streamContent = $result['stream'] ? $result['stream'] : '';
            if ($streamLength > 0)
            {
...
//设置保存到服务器本地的文件名
$filename = __DIR__ . '/APKM/';

//设置下载文件的url
$downUrl = !empty($argv[1]) ? $argv[1] : '';

if (empty($downUrl)) {
    $downUrl = isset($_GET["url"]) ? urldecode($_GET["url"]) : '';
}
...
try
{
    ...
    //开始下载
    $download->setUrl($downUrl)->setBurst($burst)->saveFile($localFileName);
}
catch (Exception $exception)
{
    file_put_contents(__DIR__ . '/download.log', $exception->getMessage() . "\r\n", FILE_APPEND);
}
