...
class Validator
{
    ...
    private $_errorTpl = [
        ...
        // 'account_database_unique' => '用户名已存在',
        // 'mobile_database_unique' => '手机已存在'
    ];
     ...
     * @param callable $func
     */
    public function setTagMap($tag, callable $func, $message = '')
    {
        $this->_tagMap[$tag] = $func;
        if ($message) {
            $this->_errorTpl[$tag] = $message;
        }
    }
    ...
    private function _perform($data, $rules)
    {
        foreach ($rules as $field => $rule) {
            ...
            if (is_array($rule)) {
                ...
                $data[$field] = $this->_perform($data[$field], $rule);
            } else {
     ...
     * @return array
     */
    private function _parseOneRule($rule)
    {
        $label = $msg = '';
    ...
    private function _executeOneRule($data, $field, $rules = [], $label = '', $msg = '')
    {
        ...
        foreach ($rules as $rule) {
            ...
            //same只能是同一层级的对比
            if ($rule == 'same') {
                $param[1] = $data[$param[1]] ? $data[$param[1]] : null;
            }
    ...
    private function _getDataByRules($rules, $data)
    {
        ...
        foreach ($rules as $field => $rule) {
            if (is_array($rule)) {
                $data[$field] = $data[$field] ? $data[$field] : null;
                $result[$field] = $this->_getDataByRules($rule, $data[$field]);
            } else {
     ...
     * @return bool
     */
    public function same($var, $compare_var)
    {
     ...
     * @return bool
     */
    public static function len($var, $len)
    {
        $len = intval($len);
     ...
     * @return bool
     */
    public static function minlen($var, $len)
    {
        $len = intval($len);
        return (mb_strlen($var) < $len) ? false : true;
    }
     ...
     * @return bool
     */
    public static function maxlen($var, $len)
    {
        $len = intval($len);
        return (mb_strlen($var) > $len) ? false : true;
    }
     ...
     * @return bool
     */
    public static function width($var, $len)
    {
        $len = intval($len);
     ...
     * @return bool
     */
    public static function minwidth($var, $len)
    {
        $len = intval($len);
        return (mb_strwidth($var) < $len) ? false : true;
    }
     ...
     * @return bool
     */
    public static function maxwidth($var, $len)
    {
        $len = intval($len);
        return (mb_strwidth($var) > $len) ? false : true;
    }
     ...
     * @return boolean
     */
    public static function isIp($var, $type = 'ipv4')
    {
     ...
     * @return    bool
     */
    public static function isNaturalNoZero($var)
    {
     ...
     * @return bool
     */
    public static function isHanzi($var)
    {
     ...
     * @return bool
     */
    public static function isMongoid($var)
    {
     ...
     * @return bool
     */
    public static function isAlphaNum($var)
    {
     ...
     * @return bool
     */
    public static function isAlphaNumDash($var)
    {
     ...
     * @return string
     */
    public static function filterUtf8($var)
    {
        /*utf8 编码表：
